...
static gpointer cc_new_frame_scrolled();
...
static gpointer cc_new_anim_scrolled(){
	...
	GtkWidget *table = gtk_table_new(1024, 1, TRUE);
...
static gpointer cc_new_frame_pane(){
	...
	GtkWidget *frame_scrolled = cc_new_frame_scrolled();
	GtkWidget *canvas_pane = cc_new_canvas_pane();
	
	gtk_paned_add1(GTK_PANED(tmp_frame_pane), frame_scrolled);	
...
static gpointer cc_new_frame_scrolled(){
	...
	GtkWidget *tmp_frame_scrolled = gtk_scrolled_window_new(NULL, NULL);
	gtk_container_set_border_width(GTK_CONTAINER(tmp_frame_scrolled), 5);
	gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(tmp_frame_scrolled),
		GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
	
	GtkWidget *table = gtk_table_new(10, 1, TRUE);
	GtkWidget *frame_add_button0 = gtk_button_new_with_label("Frame");
	GtkWidget *frame_add_button1 = gtk_button_new_with_label("Frame");
	GtkWidget *frame_add_button2 = gtk_button_new_with_label("+");
	
	gtk_table_attach_defaults(GTK_TABLE(table), frame_add_button0, 0, 1, 0, 1);
	gtk_table_attach_defaults(GTK_TABLE(table), frame_add_button1, 0, 1, 1, 2);
	gtk_table_attach_defaults(GTK_TABLE(table), frame_add_button2, 0, 1, 2, 3);
	
	gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(tmp_frame_scrolled), table);
	
	return tmp_frame_scrolled;
}
